...
2026-09-18：已接通 ROS A* → `/cmd_vel` → Unity 物理车轮 → `/odom` 的完整空载到 P1 测试，实测通过。最新启动步骤、结果和边界见 [ROS–Unity 联合测试 SOP](ROS_UNITY_TEST.md)。驱动现使用左右轮位置的刚体前向速度作 PI 反馈，`/joint_states` 仍输出 WheelCollider 编码器转速；下文较早的纯轮速比例控制和“尚未联调”条目属于历史记录。

2026-09-11：新增基于当前保存场景的 A* 仿真地图导出、分层机器人定义、障碍/目标配置及参考规划器。
空载/升高空载/载货均提供八朝向配置空间，详情和验证记录见 [规划地图说明](PLANNING_MAP.md)。

> 文档性质：项目内部开发与运维记录  
> 项目：Load-Aware Warehouse Robot  
> 当前阶段：Milestone 5 — 载货状态感知规划与路径执行
> 最后维护日期：2026-09-19
> 当前状态：仓库、物理差速底盘、举升闭环、分层规划地图和 load-aware A* 已实现；空载 Start→P1 已通过 ROS 2 + Unity 物理闭环，载货取放任务仍待完成
...
2026-09-10 仓库环境补充：已生成独立 `WarehouseEnvironment.unity`，包含物理轮机器人、三排货架、P1/P2、高低净空通道与临时障碍；已通过 Unity 批处理生成和静态碰撞净空检查。布局、坐标和尚未实现的取放货功能见 [仓库环境说明](WAREHOUSE_ENVIRONMENT.md)。以下机器人联调历史记录继续保留。

2026-09-10 机器人建模补充：两个场景已优化为功能结构模型，修正视觉轮直径为 0.36 m，增加随平台伸缩的双侧举升柱、轮毂转动标记和承载面；详细尺寸、验证与物理模型边界见 [机器人模型说明](ROBOT_MODEL.md)。下文早期视觉轮缩放记录属于历史配置。

2026-09-10 货架需求修订：仓库已重建为架下通行结构。P1/P2 低入口货架中部可举升并从侧面搬运，高货架底部整段净高 2.4 m；旧版隔墙通道与传统低层货板已移除。任务点坐标与最新检查结果以 [仓库环境说明](WAREHOUSE_ENVIRONMENT.md) 为准。

2026-09-10 静态输送交互台：P1/P2 改为中间净宽 1.12 m 的双履带镂空承托结构，轨道顶面 0.82 m；增加静态导轨、滑座、滚筒与驱动外壳。按要求设备不增加动画或动力学，详见 [静态输送台](STATIC_CONVEYOR.md)。
...
1. ~~完成物理轮、直行、转向、制动与轮速反馈联调；~~
2. ~~构建仓库场景和已知分层地图；~~
3. ~~实现 empty/raised-empty/loaded 规划配置与 load-aware A*；~~
4. ~~实现空载 Start→P1 的 ROS 2 + Unity 物理路径跟踪；~~
5. 实现货物连接、释放与载货状态确认；
6. 执行载货 P1→P2 路径并完成升降动作；
7. 实现取货—运输—放货任务状态机；
8. 添加二维雷达或降级前向检测，实现停车；
9. 时间允许时加入临时障碍重规划；
10. 完成重复性测试、README、演示视频和交付材料。
